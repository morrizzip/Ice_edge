...
def download_images(base_url, user, pwd, ukey, ids, common_params, save_dir='data/photos', max_images=3):
    ...
    for i, snapshot_id in enumerate(ids[:max_images], start=1):
        ...
        try:
            ...
            print(f"Ошибка для ID {snapshot_id}: {e}")


def main():
    print("=== Программа загрузки спутниковых снимков ===")

    MAU_USER = os.environ.get('MAU_USER')
    MAU_PASSWORD = os.environ.get('MAU_PASSWORD')
    MAU_UKEY = os.environ.get('MAU_UKEY')
    ...
    try:
        response = requests.get(metadata_url, params=metadata_params, auth=(MAU_USER, MAU_PASSWORD))
        response.raise_for_status()
        data = response.json()
    except requests.exceptions.RequestException as e:
